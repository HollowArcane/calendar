...
class CalendarBuilder
{
    ...
    buildToolbar(type, customButtons)
    {
        const toolbarContainer = this.calendar.querySelector(`${ type }`);
        if(!toolbarContainer)
        { return undefined; }
        ...
        {
            const container = toolbarContainer.querySelector(`${ type } > div[align="${ align }"]`);
            if(!container)
            { continue; }
            ...
            for (const btnGroup of container.querySelectorAll(`div[align="${align}"] > div`))
            {
                btnGroupLine = this.readBtnLine(customButtons, btnGroup.querySelectorAll('div > button, div > title'));
                if(btnGroupLine !== "")
                { btnLine.push(btnGroupLine); }
            }

            btnGroupLine = this.readBtnLine(customButtons, container.querySelectorAll(`div[align="${ align }"] > button, div[align="${ align }"] > title`));
            if(btnGroupLine !== "")
            { btnLine.push(btnGroupLine); }

            toolbar[align] = btnLine.join(' ');
        }

        let btnGroupLine = this.readBtnLine(customButtons, toolbarContainer.querySelectorAll(`${ type } > button, ${ type } > title`));
        if(btnGroupLine !== '')
        { toolbar.start = (toolbar.start ? toolbar.start + ' ': '') + btnGroupLine; }

        return toolbar;
    }

    readBtnLine(customButtons, elements)
    {
        const btnGroupLine = [];

        for (const btn of elements)
        {   
            if(btn.tagName.toLowerCase() === 'title')
            { btnGroupLine.push('title'); }
            else if(btn.getAttribute('type') === 'next' || btn.getAttribute('type') === 'prev')
            { btnGroupLine.push(btn.getAttribute('type')); }
            else if(btn.getAttribute('type') === 'next-year')
            { btnGroupLine.push('nextYear'); }
            else if(btn.getAttribute('type') === 'prev-year')
            { btnGroupLine.push('prevYear'); }
            else if(btn.getAttribute('type') === 'today')
            { btnGroupLine.push('today'); }
            else
            {
                const btnID = `btn${++this.btnId}`;
                customButtons[btnID] = btn;
    
                btnGroupLine.push(btnID);
            }
        } 

        return btnGroupLine.join(',')
    }
    ...
    createCustomButtons(customButtons)
    {
        const buttons = {};
        for(let key in customButtons)
        {
            const type = this.buildType(
                customButtons[key].getAttribute('type'),
                customButtons[key].getAttribute('track-by'),
                customButtons[key].getAttribute('track-over')
            );

            buttons[key] = {
                text: customButtons[key].textContent,
                click: e => {
                    if(customButtons[key].getAttribute('type'))
                    { this.fullCalendar.changeView(type); }
                    
                    customButtons[key].click(e);
                }
            };
        }
    ...
    build()
    {
        const DOMeventsWrap = this.calendar.querySelector('events')
        ...
        const maxEvents = DOMeventsWrap.getAttribute('max-events');

        const clickEvent = this.calendar.onclick;
        this.calendar.onclick = undefined;

        const selectEvent = this.calendar.onselect;
        this.calendar.onselect = undefined;

        const customButtons = {};
        const calendarInfo = {
            dayMaxEvents: maxEvents ? parseInt(maxEvents): false,
            headerToolbar: this.buildToolbar('header', customButtons),
            footerToolbar: this.buildToolbar('footer', customButtons),
            customButtons: this.createCustomButtons(customButtons),
            height: this.calendar.getAttribute('height'),
            initialView: type,
            events: events,
        ...
        this.fullCalendar = new FullCalendar.Calendar(this.canvas, calendarInfo);
...
window.addEventListener('load', () => {
    ...
    for(const calendar of calendars) {
        ...
        if(builder)
        {
            const id = calendar.getAttribute("id")
            const instance = builder.build();
            instance.render();
            
            if(id)
            { CALENDARS[id] = instance }
        }
    }
})